(function ($) {
    Drupal.behaviors.contextReactionBlock = {
    ...
    DrupalContextBlockForm = function (blockForm) {
        ...
        // by tabledrag
        $('#ctools-export-ui-edit-item-form').submit(function () {
            Drupal.contextBlockForm.setState();
        });
    ...
    DrupalContextBlockEditor.prototype = {
        ...
        disableTextSelect: function () {
            if ($.browser.safari) {
                ...
                $('.block:has(a.context-block):not(:has(input,textarea))').bind('selectstart.contextBlockEditor', function () {
                    return false;
                });
            }
            else {
                $(this).bind('mousedown.contextBlockEditor', function () {
                    return false;
                });
            }
        },
            ...
            $(this.regions).each(function () {
                ...
                var params = {
                    ...
                    start: function (event, ui) {
                        self.scriptFix(event, ui, editor, context);
                    },
                    stop: function (event, ui) {
                        self.addBlock(event, ui, editor, context);
                    },
                    receive: function (event, ui) {
                        self.addBlock(event, ui, editor, context);
                    },
                    over: function (event, ui) {
                        self.updateRegion(event, ui, region, 'over');
                    },
                    out: function (event, ui) {
                        self.updateRegion(event, ui, region, 'out');
                    },
